...
log "info" "3. Checking for existing Docker installation"
if dpkg -l | grep -q 'docker'; then
    log "info" "Docker is already installed. Skipping removal of existing Docker packages."
else
    log "info" "Removing conflicting Docker packages if any"
...
read -p "Nama Validator: " validator_name
read -p "Address EVM: " wallet_pub_key
read -p "Private Key Wallet: " wallet_priv_key
